...
public class StoredTreeStructureVerifier {
    ...
    public static void testUnOrderedTreeStructure1(IndexStorageManager indexStorageManager, int table, long multi, int degree) throws ExecutionException, InterruptedException {
        ...
        Assertions.assertEquals(multi * 9, rootNode.getKeys().next());

        // Checking root child at left
        InternalTreeNode leftChildInternalNode = (InternalTreeNode) TreeNodeIO.read(indexStorageManager, table, ((InternalTreeNode) rootNode).getKeyPointersList().get(0).getLeft());
        List<Long> leftChildInternalNodeKeys = leftChildInternalNode.getKeyList();
        ...
        // Far left leaf
        LeafTreeNode currentLeaf = (LeafTreeNode) TreeNodeIO.read(indexStorageManager, table, leftChildInternalNode.getKeyPointersList().get(0).getLeft());
        List<Long> currentLeafKeys = currentLeaf.getKeyList();
        ...
        Assertions.assertEquals(nextPointer.get(), leftChildInternalNode.getKeyPointersList().get(0).getRight());

        currentLeaf = (LeafTreeNode) TreeNodeIO.read(indexStorageManager, table, leftChildInternalNode.getKeyPointersList().get(0).getRight());
        currentLeafKeys = currentLeaf.getKeyList();
        ...
        System.out.println(
                HashCode.fromBytes(Arrays.copyOfRange(currentLeaf.toBytes(), 1 + ((degree - 1) * (Pointer.BYTES + Long.BYTES)), currentLeaf.toBytes().length))
        );

        System.out.println(currentLeaf.getPointer());
        Assertions.assertTrue(nextPointer.isPresent());
        Assertions.assertEquals(nextPointer.get(), leftChildInternalNode.getKeyPointersList().get(1).getRight());

        currentLeaf = (LeafTreeNode) BaseTreeNode.fromBytes(
                indexStorageManager.readNode(
                        table,
                        leftChildInternalNode.getKeyPointersList().get(1).getRight()
                ).get().bytes()
        );
        currentLeafKeys = currentLeaf.getKeyList();
        Assertions.assertEquals(3, currentLeafKeys.size());
        ...
        InternalTreeNode rightChildInternalNode = (InternalTreeNode) BaseTreeNode.fromBytes(
                indexStorageManager.readNode(
                        table,
                        ((InternalTreeNode) rootNode).getKeyPointersList().get(0).getRight()).get().bytes()
        );
        List<Long> rightChildInternalNodeKeys = rightChildInternalNode.getKeyList();
        ...
        nextPointer = currentLeaf.getNextSiblingPointer(degree);
        Assertions.assertTrue(nextPointer.isPresent());
        Assertions.assertEquals(nextPointer.get(), rightChildInternalNode.getKeyPointersList().get(0).getLeft());

        currentLeaf = (LeafTreeNode) BaseTreeNode.fromBytes(
                indexStorageManager.readNode(
                        table,
                        rightChildInternalNode.getKeyPointersList().get(0).getLeft()
                ).get().bytes()
        );
        currentLeafKeys = currentLeaf.getKeyList();
        Assertions.assertEquals(2, currentLeafKeys.size());
        ...
        nextPointer = currentLeaf.getNextSiblingPointer(degree);
        Assertions.assertTrue(nextPointer.isPresent());
        Assertions.assertEquals(nextPointer.get(), rightChildInternalNode.getKeyPointersList().get(0).getRight());

        currentLeaf = (LeafTreeNode) BaseTreeNode.fromBytes(
                indexStorageManager.readNode(
                        table,
                        rightChildInternalNode.getKeyPointersList().get(0).getRight()
                ).get().bytes()
        );
        currentLeafKeys = currentLeaf.getKeyList();
        Assertions.assertEquals(2, currentLeafKeys.size());
    ...
    public static void testOrderedTreeStructure(IndexStorageManager indexStorageManager, int table, long multi, int degree) throws ExecutionException, InterruptedException {
        ...
        Assertions.assertEquals(multi*7, rootNode.getKeys().next());

        // Checking root child at left
        InternalTreeNode leftChildInternalNode = (InternalTreeNode) BaseTreeNode.fromBytes(
                indexStorageManager.readNode(
                        table,
                        ((InternalTreeNode) rootNode).getKeyPointersList().get(0).getLeft()).get().bytes()
        );
        List<Long> leftChildInternalNodeKeys = leftChildInternalNode.getKeyList();
        ...
        LeafTreeNode currentLeaf = (LeafTreeNode) BaseTreeNode.fromBytes(
                indexStorageManager.readNode(
                        table,
                        leftChildInternalNode.getKeyPointersList().get(0).getLeft()
                ).get().bytes()
        );
        List<Long> currentLeafKeys = currentLeaf.getKeyList();
        Assertions.assertEquals(2, currentLeafKeys.size());
        ...
        currentLeaf = (LeafTreeNode) BaseTreeNode.fromBytes(
                indexStorageManager.readNode(
                        table,
                        leftChildInternalNode.getKeyPointersList().get(0).getRight()
                ).get().bytes()
        );
        currentLeafKeys = currentLeaf.getKeyList();
        Assertions.assertEquals(2, currentLeafKeys.size());
        ...
        nextPointer = currentLeaf.getNextSiblingPointer(degree);
        Assertions.assertTrue(nextPointer.isPresent());
        Assertions.assertEquals(nextPointer.get(), rightChildInternalNode.getKeyPointersList().get(1).getRight());

        currentLeaf = (LeafTreeNode) BaseTreeNode.fromBytes(
                indexStorageManager.readNode(
                        table,
                        rightChildInternalNode.getKeyPointersList().get(1).getRight()
                ).get().bytes()
        );
        currentLeafKeys = currentLeaf.getKeyList();
        Assertions.assertEquals(2, currentLeafKeys.size());
